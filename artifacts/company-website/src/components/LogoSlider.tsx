    ...
    <section className="py-10 bg-white border-y border-gray-100">
      <style>{`
        @keyframes sin-scroll {
          ...
          100% { transform: translateX(calc(-${SLIDE_W}px * ${LOGOS.length})); }
        }
        .sin-slide-track {
          display: flex;
          width: calc(${SLIDE_W}px * ${doubled.length});
          animation: sin-scroll ${SPEED} linear infinite;
          will-change: transform;
        }
        @media (max-width: 768px) {
          .sin-slide-track {
            width: calc(170px * ${doubled.length});
            animation: sin-scroll-mobile 18s linear infinite;
          }
          @keyframes sin-scroll-mobile {
            0%   { transform: translateX(0); }
            100% { transform: translateX(calc(-170px * ${LOGOS.length})); }
          }
        }
      `}</style>
