...
<h1 style="font-size:28px;font-weight:900;color:#111827;margin:0;">請&nbsp;求&nbsp;書</h1></div>
<div style="text-align:right;font-size:12px;color:#6b7280;line-height:2;">
<p style="margin:0;">請求書番号: <strong>${inv.invoiceNo}</strong></p>
<p style="margin:0;">請求日: ${inv.date}</p>
<p style="margin:0;">支払期限: <strong>${inv.dueDate}</strong></p>
</div></div>
<div style="background:#f3f4f6;padding:16px 20px;margin-bottom:32px;">
<p style="margin:0;font-size:14px;color:#374151;">${inv.company}&nbsp; ${inv.client}&nbsp;様</p>
</div>
...
${inv.notes ? `<div style="border-left:3px solid #e5e7eb;padding-left:16px;font-size:13px;color:#6b7280;margin-bottom:32px;"><p style="font-weight:bold;margin:0 0 6px;">備考</p><p style="margin:0;">${inv.notes}</p></div>` : ""}
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 border border-white/10 w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
          <h3 className="font-black">請求書プレビュー — {invoice.invoiceNo}</h3>
        ...
        {sent ? (
          ...
          <>
            <div className="flex-1 overflow-auto p-4">
              <iframe srcDoc={html} className="w-full min-h-[500px] bg-white border-0" title="invoice preview" />
            </div>
            <div className="px-6 py-4 border-t border-white/10 flex items-center justify-between">
              <p className="text-xs text-gray-500">送信先: <span className="text-white">{invoice.email}</span></p>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="border border-white/10">
        <div className="grid grid-cols-6 px-4 py-2 border-b border-white/10 text-[10px] text-gray-500 tracking-widest uppercase">
          <span>番号</span><span>顧客</span><span>支払期限</span><span className="text-right">金額</span><span className="text-right">状態</span><span className="text-right">操作</span>
        </div>
